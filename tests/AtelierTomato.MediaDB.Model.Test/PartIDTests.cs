...
namespace AtelierTomato.MediaDB.Model.Test
{
	public class PartIDTests
	{
		...
		[Fact]
		public void PartIDParseDecimalAndNegativeTest()
		{
			var partIDString = "1:2.5:-3";
			var partID = PartID.Parse(partIDString);
			partID.Should().BeEquivalentTo(new PartID(-3, new PartID(2.5M, new PartID(1))));
		}
		[Fact]
		public void PartIDToStringDecimalAndNegativfeTest()
		{
			var partID = new PartID(-3, new PartID(2.5M, new PartID(1)));
			var partIDString = partID.ToString();
			partIDString.Should().Be("1:2.5:-3");
		}
		[Fact]
		public void PartIDNotIntegerTest()
		{
			Action act = () => PartID.Parse("3:a:1");
			act.Should().Throw<ArgumentException>().WithMessage($"{nameof(PartID)} failed to parse as 'a' is not a valid decimal value. (Parameter 'input')");
		}
		[Fact]
